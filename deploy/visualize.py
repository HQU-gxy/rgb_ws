...
def visualize_normalized_pose(imgfile,
    ...
    kpt_nums = len(normalized_keypoints)
    print("==============kpt_nums:", kpt_nums)
    ...
    # 计算缩放比例，即新的高度
    target_height = img_height / img_width
    # 由于我们不能有宽度为1像素的图像，我们将高度相对于新的宽度进行缩放
    # 以保持宽高比，这里我们选择新宽度为100像素以便可视化
    new_width = 100
    new_height = int(target_height * new_width)
    ...
    for keypoint in normalized_keypoints:
        x, y = int((keypoint[0]) * 1000), int((keypoint[1]) * 1000)
        cv2.circle(canvas, (x, y), 5, (0, 255, 0), thickness=-1)
    
    # 绘制骨架连线
    for start, end in EDGES:
        if start < len(normalized_keypoints) and end < len(normalized_keypoints):
            start_point = (int((normalized_keypoints[start][0]) * 1000), int((normalized_keypoints[start][1]) * 1000))
            end_point = (int((normalized_keypoints[end][0]) * 1000), int((normalized_keypoints[end][1]) * 1000))
            cv2.line(canvas, start_point, end_point, (255, 255, 255), 2)
    
    save_name = os.path.join(
        save_dir, os.path.splitext(os.path.basename(imgfile))[0] + '_normalized_vis.jpg')
